...
class OutputLayer(nn.Module):
    def __init__(self, args, Y, dicts, input_size):
        ...
        self.loss_function = FocalLoss(args.alpha, args.gamma, args.reduce)#nn.BCEWithLogitsLoss()
...
class OutputLayer_MTL(nn.Module):
    def __init__(self, args, Y, Y_ccs, dicts, input_size):
        ...
        self.loss_function = FocalLoss(args.alpha, args.gamma, args.reduce)#nn.BCEWithLogitsLoss()
        if args.RAM:
            self.ram = RAM(3, 1, input_size, args)

    def forward(self, x, target, target_ccs, text_inputs):
        try:
            x = self.ram(x.transpose(1, 2)).transpose(1, 2)
